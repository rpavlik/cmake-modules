...
function(pedantic_compiler_flags _language _pedantic_flags)
    ...
    elseif(${_language}_COMPILER_IS_GNU OR ${_language}_COMPILER_IS_CLANG)
        ...
        message(WARNING "unsuported toolchain ${CMAKE_${_language}_COMPILER_ID}")
        return()
    endif()
...
function(language_dialect_flags _language _dialect _language_dialect_flags)
    if(${_language} STREQUAL "C" AND (${_dialect} STREQUAL "C90" OR ${_dialect} STREQUAL "C99" OR ${_dialect} STREQUAL "C11"))
        string(TOLOWER ${_dialect} std)
    elseif(${_language} STREQUAL "C" AND (${_dialect} STREQUAL "C89" OR ${_dialect} STREQUAL "ANSI"))
        set(std "c90")
    elseif(${_language} STREQUAL "CXX" AND (${_dialect} STREQUAL "C++98" OR ${_dialect} STREQUAL "C++03" OR ${_dialect} STREQUAL "C++11"))
        string(TOLOWER ${_dialect} std)
    elseif(${_language} STREQUAL "CXX" AND ${_dialect} STREQUAL "ANSI")
        set(std "c++03")
    else()
        message(WARNING "unsuported dialect ${_dialect} of language ${_language}")
        return()
    endif()

    if(${_language}_COMPILER_IS_GNU OR ${_language}_COMPILER_IS_CLANG)
        set(${_language_dialect_flags} "-std=${std}")
    else()
        message(WARNING "unsuported toolchain ${CMAKE_${_language}_COMPILER_ID}")
        return()
    endif()

    check_compile_flags(${_language} "${${_language_dialect_flags}}" ${_language_dialect_flags})
